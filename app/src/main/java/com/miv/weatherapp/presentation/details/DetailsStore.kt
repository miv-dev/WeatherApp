...
import com.miv.weatherapp.domain.usecase.ChangeFavouriteStateUseCase
import com.miv.weatherapp.domain.usecase.GetForecastUseCase
import com.miv.weatherapp.domain.usecase.ObserveFavouriteStateUseCase
import com.miv.weatherapp.presentation.details.DetailsStore.Intent
import com.miv.weatherapp.presentation.details.DetailsStore.Label
import com.miv.weatherapp.presentation.details.DetailsStore.State
...
class DetailsStoreFactory @Inject constructor(
    ...
    private sealed interface Action {
        ...
        data class FavouriteStateChanged(val isFavourite: Boolean) : Action
    ...
    private sealed interface Msg {
        ...
        data class FavouriteStateChange(val isFavourite: Boolean) : Msg
        ...
        override fun executeAction(action: Action, getState: () -> State) {
            ...
            when(action){
                is Action.FavouriteStateChanged -> {
                    dispatch(Msg.FavouriteStateChange(action.isFavourite))
                }
    ...
    private object ReducerImpl : Reducer<State, Msg> {
        override fun State.reduce(msg: Msg) = when(msg){
            is Msg.FavouriteStateChange -> copy(isFavourite = msg.isFavourite)
            is Msg.ForecastLoaded -> copy(forecastState = State.ForecastState.Loaded(msg.forecast))
            is Msg.ForecastStartLoading -> copy(forecastState = State.ForecastState.Loading)
            is Msg.ForecastLoadingError -> copy(forecastState = State.ForecastState.Error)
        }
    }
}
